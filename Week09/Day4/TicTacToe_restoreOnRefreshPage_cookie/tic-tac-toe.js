...
window.onload = () => {
   ...
   restoreCookiesAfterRefreshPage();
}
...
window.onbeforeunload= () => { 
  let cookie = '';
    ...
    document.cookie = 'move='+ move;
    document.cookie = 'endGame='+ endGame;
    document.cookie = 'isGameCancel=' + isGameCancel;
    document.cookie = 'playedPositions=' + JSON.stringify(playedPositions);
}

function restoreCookiesAfterRefreshPage() {
    /*Problem: endGame chiar dc e false  in cookie in fc handlerClickOnBoard e true !!!*/ 
    let cookie = document.cookie;
 

 if (cookie) {
    let cookieArry = cookie.split('; ');
    move = parseInt(cookieArry.find(e=> e.startsWith('move=')).split('=')[1]);
    if (move > 0) {
        endGame = cookieArry.find(e=> e.startsWith('endGame=')).split('=')[1] === 'true' ? true : false;
        isGameCancel = cookieArry.find(e=> e.startsWith('isGameCancel=')).split('=')[1]=== 'true' ? true : false;
        playedPositions = JSON.parse(cookieArry.find(e=> e.startsWith('playedPositions=')).split('=')[1]);
            
        for(let i = 0; i < n; i++) {
            for(let j = 0; j < n; j++) {
                let elem =  playedPositions[i][j];
                let img = '';

                if (elem === '') continue;
                img = elem === 'x' ? "./img/player-x.svg" : "./img/player-o.svg";
                document.getElementById(`img_${i}${j}`).src = img;
            }
        }
    }
    updateStatusBoard();
