...
function restoreTheme() {
    // Your code here
    const themeName = getValueOfACookie('themeName');
...
function clearTheme() {
    ...
    //solution2
    //const dateExpires = new Date(1, 1, 1977).toUTCString();
    //document.cookie =`themeName=''; expires=' + ${dateExpires}`;

    //solution 3
    //document.cookie = "themeName=''; expires=Thu, 01 Jan 1970 00:00:01 GMT";
...
function storeName(displayName) {
    // Your code here
    const display = "displayName=" + displayName;
...
function restoreName() {
    // Your code here
    const displayName = getValueOfACookie('displayName');
...
function getValueOfACookie (cookieName) {
    const cookie = document.cookie;
    const arryCookies = cookie.split(';');
    value = arryCookies.find(e => e.trim().startsWith(cookieName + '='));
